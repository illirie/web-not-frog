...
import { Movie, validateData, generateId } from "./simple/simple";
    ...
    return (
        <div className="movie-form__block column">
            ...
            <form id="card-form" className="movie-form">
                ...
                <Typography>Ссылка на обложку</Typography>
                ...
                <textarea className="movie-form__input-description" name="description" cols="20" rows="5" value={plotInput} onChange={(e) => setPlot(e.target.value)} placeholder="Введите описание" />
                ...
                <Rating name="rating" value={ratingInput} precision={0.5} onChange={(e) => setRating(e.target.value)} required></Rating>
                <input className="movie-form__input" name="rating" type="number" value={ratingInput} onChange={(e) => setRating(e.target.value)} placeholder="Задайте рейтинг" readOnly />
                ...
                <div className="row" style={{marginLeft: "auto", marginRight: 0}}>
                    ...
                    <Link to={addPage ? `/movie/${newMovieId}` : `/movie/${param.imdbID}`}>
                        <Button onClick={addPage ? addMovie : editMovie}>Сохранить</Button>
                    </Link>
                </div>
            </form>
        </div>
